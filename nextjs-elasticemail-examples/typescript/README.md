# Next.js Email API Examples (TypeScript) - Elastic Email

Send transactional and bulk email from Next.js 15 (App Router) with the [Elastic Email](https://elasticemail.com/email-api) email API, in TypeScript. Every call runs in a Route Handler on the server, so the API key never ships to the client. The [JavaScript variant](../javascript/) is identical in behaviour.
...
cd elasticemail-examples/nextjs-elasticemail-examples/typescript
...
src/
  app/
    ...
    layout.tsx                 Nav
    page.tsx                   Index of examples
    globals.css                Plain CSS
  components/
    page-header.tsx, result-display.tsx, code-block.tsx
  lib/
