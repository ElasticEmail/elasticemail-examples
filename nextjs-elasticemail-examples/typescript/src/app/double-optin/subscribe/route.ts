...
import { apiError, contactsApi, emailsApi, from, hmac, publicUrl } from "@/lib/elasticemail";
...
export async function POST(request: Request) {
  ...
  <a href="${confirmUrl}" style="background-color: #18181b; color: #fff; padding: 12px 32px; border-radius: 6px; text-decoration: none; font-weight: bold; display: inline-block;">Confirm Subscription</a>
    ...
    return NextResponse.json({ success: true, message: "Confirmation email sent", messageId: data.MessageID });
  } catch (err) {
