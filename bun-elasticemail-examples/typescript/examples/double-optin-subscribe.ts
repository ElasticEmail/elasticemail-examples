...
// Usage: bun run examples/double-optin-subscribe.ts user@example.com "John Doe"
const [email, name = ""] = process.argv.slice(2);

if (!email) {
  console.error('Usage: bun run examples/double-optin-subscribe.ts <email> ["Name"]');
  process.exit(1);
}
...
try {
  ...
  console.log("\nWhen the link is opened, GET /double-optin/confirm in src/index.ts adds the contact to the list.");
} catch (err: any) {
