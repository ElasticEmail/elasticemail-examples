...
2. Fetch the use-case map:
   ...
   Pick the stack by `id` (`nextjs`, `nodejs` for Node.js with no framework, `nestjs`, `fastify`, `kotlin`, `laravel`, `python`, `go`, `serverless-cloudflare-workers`, ...)
...
5. Outside the use-case map: `smtp-elasticemail-examples/<service>/` when a tool only takes SMTP
   settings, `email-templates-elasticemail-examples/` for React Email or MJML,
   `ai-agents-elasticemail-examples/` for an LLM tool, `queues-elasticemail-examples/` for sending
