...
export const Route = createFileRoute("/inbound")({
  ...
  head: () => ({ meta: [{ title: "Inbound - Elastic Email Examples" }] }),
});

const exampleCode = `// src/routes/api/inbound.ts
...
const exampleCode3 = `curl -X POST "http://localhost:3000/api/inbound?token=change_me" \\
...
function InboundPage() {
  return (
    <main>
      <PageHeader title="Inbound Emails" description="Receive parsed emails through an Elastic Email inbound route and forward them." sourcePath="src/routes/api/inbound.ts" />

      <div className="card">
        <h3>Setup Steps</h3>
        <ol>
          <li>Point the MX record of <code>SENDING_DOMAIN</code> at <code>mx.inbound.elasticemail.com</code>.</li>
          <li>Expose the dev server (for example <code>ngrok http 3000</code>) and set <code>PUBLIC_URL</code> to the HTTPS URL.</li>
          <li>Create an inbound route with action <code>NotifyViaHttp</code> and address <code>PUBLIC_URL/api/inbound?token=ELASTICEMAIL_WEBHOOK_TOKEN</code>, either in the dashboard or with the code below.</li>
