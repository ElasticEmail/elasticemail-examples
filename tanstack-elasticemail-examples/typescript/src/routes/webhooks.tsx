...
function WebhooksPage() {
  return (
    <main>
      <PageHeader title="Webhooks" description="Receive delivery and engagement events from Elastic Email." sourcePath="src/routes/api/webhook.ts" />

      <div className="card">
        <h3>Setup Steps</h3>
        <ol>
          <li>Expose the dev server (for example <code>ngrok http 3000</code>) and set <code>PUBLIC_URL</code>.</li>
          <li>In the Elastic Email dashboard (Settings, Notifications) add a webhook with URL <code>PUBLIC_URL/api/webhook?token=ELASTICEMAIL_WEBHOOK_TOKEN</code>, or create it with the code below.</li>
