# Node.js Email API Examples - Elastic Email

Send transactional and bulk email from plain Node.js with the [Elastic Email](https://elasticemail.com/email-api) email API. There's no web framework: the server app is built on the `node:http` module, and every example calls the official Elastic Email TypeScript SDK directly. Use it as a reference when your project has its own server, or none at all: a CLI, a queue worker or a cron job. TypeScript and JavaScript variants are included.

> **First time here?** The [Node.js quickstart](QUICKSTART.md) gets you from nothing to a delivered email in five minutes.
> For the concepts behind these examples, see the [Elastic Email guides](../docs/README.md).

**Versions:** Elastic Email REST API v4 · SDK `@elasticemail/elasticemail-client-ts-axios@4.2.0` · Node.js 20+ · node:http
...
The only runtime dependencies are the SDK and `dotenv`. The TypeScript variant adds `tsx` and
...
`src/index.ts` is a single `http.createServer()` handler. It parses the URL with `new URL()`,
reads the body with `for await (const chunk of req)`, parses JSON or form-encoded bodies, and
routes on `"<METHOD> <path>"` in one `switch`.
...
(`crypto.timingSafeEqual`). Elastic Email sends each event as a GET request with the details in the
query string: `status` (Sent, Opened, Clicked, Error, AbuseReport, Unsubscribed), `to`,
`transaction`, `messageid`, `category`, `target`.
...
`node:http` has no body parser, so the server parses the form with `URLSearchParams` and caps the
body at 25 MB for base64 attachments.
...
```
nodejs-elasticemail-examples/
├── typescript/
│   ├── examples/                    # 18 standalone examples
│   ├── src/index.ts                 # node:http server
│   ├── package.json
│   └── tsconfig.json
├── javascript/
│   ├── examples/                    # 18 standalone examples
│   ├── src/index.js                 # node:http server
│   └── package.json
├── .env.example
└── README.md
```
...
If a tool or library in your project only speaks SMTP, see
[Nodemailer](../smtp-elasticemail-examples/nodemailer/) or the dependency-free
[Node.js SMTP client](../smtp-elasticemail-examples/nodejs/) in the SMTP section.
