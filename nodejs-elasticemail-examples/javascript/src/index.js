...
const inbound = async (res, token, mail) => {
  if (!tokenOk(token)) {
    sendJson(res, 401, { error: "Invalid token" });
    return;
  }
...
const subscribe = async (res, body) => {
...
const optinWebhook = async (res, event) => {
  if (!tokenOk(event.token)) {
    sendJson(res, 401, { error: "Invalid token" });
    return;
  }
