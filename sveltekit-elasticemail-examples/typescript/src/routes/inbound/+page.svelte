<script lang="ts">
  ...
  const handlerCode = `// src/routes/api/inbound/+server.ts
...
await inboundApi.inboundroutePost({
  ...
  Filter: \`*@\${env.SENDING_DOMAIN}\`,
  FilterType: "EmailAddress",
  ActionType: "NotifyViaHttp",
  HttpAddress: \`\${env.PUBLIC_URL}/api/inbound?token=\${env.ELASTICEMAIL_WEBHOOK_TOKEN}\`,
});`;

  const curlCode = `curl -X POST "http://localhost:5173/api/inbound?token=change_me" \\
...
<svelte:head>
  <title>Inbound - Elastic Email Examples</title>
</svelte:head>
...
<main>
  <PageHeader
    ...
    sourcePath="src/routes/api/inbound/+server.ts"
  />
  ...
  <div class="card">
    ...
    <ol>
      ...
      <li>
        Expose the dev server (for example <code>ngrok http 5173</code>) and set <code>PUBLIC_URL</code> to the HTTPS
        URL.
      </li>
      <li>
        Create an inbound route with action <code>NotifyViaHttp</code> and address
        <code>PUBLIC_URL/api/inbound?token=ELASTICEMAIL_WEBHOOK_TOKEN</code>, either in the dashboard or with the code
        below.
      </li>
  ...
  <div class="code-block"><pre><code>{routeCode}</code></pre></div>

  <h2>Test Locally</h2>
  <p class="muted">Simulate a notification with curl (uses the default token from .env.example):</p>
