...
export const contactEmail = env.CONTACT_EMAIL || from;
export const listName = env.ELASTICEMAIL_LIST_NAME || "Newsletter";
export const templateName = env.ELASTICEMAIL_TEMPLATE_NAME || "welcome-example";
export const sendingDomain = env.SENDING_DOMAIN || "yourdomain.com";
export const publicUrl = env.PUBLIC_URL || "http://localhost:5173";
export const secret = env.ELASTICEMAIL_WEBHOOK_TOKEN || "change_me";
...
export async function readEvent(request: Request): Promise<Record<string, string>> {
  const event: Record<string, string> = {};
  for (const [key, value] of new URL(request.url).searchParams) event[key] = value;
