...
useHead({ title: "Inbound - Elastic Email Examples" });

const handlerCode = `// server/api/inbound.post.ts
export default defineEventHandler(async (event) => {
  if (!tokenOk(getQuery(event).token)) {
    return fail(event, 401, "Invalid token");
  }

  // Elastic Email posts the parsed message as form fields; readBody parses them into an object
...
<template>
  <main>
    <PageHeader
      ...
      source-path="server/api/inbound.post.ts"
    />
    ...
    <div class="card">
      ...
      <ol>
        ...
        <li>
          Expose the dev server (for example <code>ngrok http 3000</code>) and set <code>PUBLIC_URL</code> to the HTTPS
          URL.
        </li>
        <li>
