...
export const sendEmailTool = tool({
  ...
  // OpenAI strict tool schemas need every field listed as required, so the optional field is nullable.
  // Keep keywords simple (no minLength): strict mode supports only part of JSON Schema.
  parameters: z.object({
    to: z.email().describe("Recipient email address"),
    subject: z.string().describe("Subject line"),
    text: z.string().describe("Plain-text body"),
    html: z.string().nullable().describe("Optional HTML body, or null to derive it from text."),
  }),
  execute: async ({ html, ...rest }) => sendEmail({ ...rest, html: html ?? undefined }),
});
