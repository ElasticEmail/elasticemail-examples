...
// LangChain sends the tool's return value to the model as the ToolMessage content, so return JSON text.
export const sendEmailTool = tool(async (input) => JSON.stringify(await sendEmail(input)), {
  name: "send_email",
  description:
