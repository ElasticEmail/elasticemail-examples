# Fastify Email API Examples - Elastic Email

Send transactional and bulk email from a Fastify 5 server on Node.js 20+ with the [Elastic Email](https://elasticemail.com/email-api) email API. Every route calls the official `@elasticemail/elasticemail-client-ts-axios` SDK, and `@fastify/formbody` parses the form-encoded webhook and inbound requests that Elastic Email posts. TypeScript and JavaScript variants are included, with identical behavior.

> **First time here?** The [Fastify quickstart](QUICKSTART.md) gets you from nothing to a delivered email in five minutes.
> For the concepts behind these examples, see the [Elastic Email guides](../docs/README.md).

**Versions:** Elastic Email REST API v4 · SDK `@elasticemail/elasticemail-client-ts-axios@4.2.0` · Node.js 20+ · Fastify 5
...
Errors come back as `{ "error": "<message>" }` with the Elastic Email API status. Unknown routes
answer 404 with `{ "error": "Not found" }`.
...
Fastify only parses JSON out of the box. The app registers `@fastify/formbody` for these form
posts and raises `bodyLimit` to 25 MB, because inbound email carries attachments as base64 fields
and Fastify's default limit is 1 MB.
...
```
fastify-elasticemail-examples/
├── typescript/
│   ├── examples/                    # 18 standalone examples
│   ├── src/index.ts                 # Fastify app
│   ├── package.json
│   └── tsconfig.json
├── javascript/
│   ├── examples/                    # 18 standalone examples
│   ├── src/index.js                 # Fastify app
│   └── package.json
├── .env.example
├── QUICKSTART.md
└── README.md
